<x-filament-panels::page>
    ...
    <div class="max-w-md mx-auto space-y-4 pb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 text-center">
            ...
            @if ($this->isHariLibur)
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 mt-2">
                    Hari Libur
                </span>
            @endif
        </div>
        ...
        @if ($this->guru)
            <div
                class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-3">
                <div
        ...
        @else
            <div
                class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-4 text-center">
            ...
            <div
                ...
                @if (!$foto_base64)
                    ...
                    <div class="relative bg-black" style="min-height: 280px;">
                        <video id="kamera-video" autoplay playsinline muted class="w-full object-cover"
                            style="min-height: 280px; display: none;"></video>
                        ...
                        <div id="kamera-placeholder" class="absolute inset-0 flex flex-col items-center justify-center">
                            <svg class="w-16 h-16 text-gray-600 mb-3" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                ...
                @else
                    ...
                    <div class="relative">
                        <img id="foto-preview" src="{{ $foto_base64 }}" alt="Foto" class="w-full object-cover"
                            style="max-height: 280px;">
                        <button wire:click="$set('foto_base64', null)" onclick="resetKamera()"
                        ...
                        <div
                            class="absolute bottom-3 left-3 bg-green-500 text-white text-xs font-medium px-2 py-1 rounded-lg flex items-center gap-1">
                ...
                <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-700 space-y-2">
                    ...
                    {{-- Status GPS --}}
                    <div id="lokasi-status" class="flex items-center gap-2">
                        <div class="w-2 h-2 rounded-full bg-gray-400 animate-pulse"></div>
                        <span class="text-xs text-gray-400">Mendeteksi lokasi...</span>
                    </div>

                    {{-- Preview peta — muncul setelah GPS berhasil --}}
                    <div id="peta-preview"
                        class="hidden rounded-xl overflow-hidden border border-gray-200 dark:border-gray-600"
                        style="height: 180px;">
    ...
    @push('scripts')
        ...
        <script>
            ...
            // ── GPS — ambil SEKALI, simpan lokal, kirim ke Livewire ─────────────────
            function deteksiLokasi() {
                ...
                updateLokasiStatus(null, 'Mendeteksi lokasi...');

                // Coba dulu dengan akurasi tinggi (GPS asli)
                navigator.geolocation.getCurrentPosition(
                    (pos) => simpanLokasi(pos),
                    (err) => {
                        // Fallback: akurasi rendah (WiFi/IP) — lebih cepat di laptop/kantor
                        navigator.geolocation.getCurrentPosition(
                            (pos) => simpanLokasi(pos),
                            (err) => updateLokasiStatus(false, 'Gagal deteksi lokasi — izinkan akses GPS'), {
            ...
            function simpanLokasi(pos) {
                const lat = pos.coords.latitude;
                const lng = pos.coords.longitude;
                const acc = Math.round(pos.coords.accuracy);

                // Kirim ke Livewire — hanya sekali
                @this.set('lat', lat);
                @this.set('lng', lng);

                updateLokasiStatus(true, `${lat.toFixed(5)}, ${lng.toFixed(5)} (±${acc}m)`);
                tampilkanPetaPreview(lat, lng);
            }

            function tampilkanPetaPreview(lat, lng) {
                const petaEl = document.getElementById('peta-preview');
                const mapEl = document.getElementById('map-preview');
                if (!petaEl || !mapEl) return;

                petaEl.classList.remove('hidden');

                if (previewMap) {
                    // Sudah ada peta — update saja posisinya
            ...
            function updateLokasiStatus(sukses, pesan) {
                ...
                if (sukses === null) {
                    // Loading state
                    el.innerHTML = `<div class="w-2 h-2 rounded-full bg-gray-400 animate-pulse"></div>
                            <span class="text-xs text-gray-400">${pesan}</span>`;
                } else if (sukses) {
                    el.innerHTML = `<div class="w-2 h-2 rounded-full bg-green-500"></div>
                            <span class="text-xs text-green-600 dark:text-green-400">${pesan}</span>`;
                } else {
                    el.innerHTML =
                        `<div class="w-2 h-2 rounded-full bg-red-500"></div>
                            <span class="text-xs text-red-500">${pesan}</span>
                            <button onclick="deteksiLokasi()" class="text-xs text-primary-500 underline ml-1">Coba lagi</button>`;
                }
            }

            // ── Midnight refresh ─────────────────────────────────────────────────────
            function jadwalkanRefreshTengahMalam() {
                const sekarang = new Date();
                const tengahMalam = new Date();
                tengahMalam.setHours(24, 0, 10, 0); // 00:00:10 hari berikutnya
                const sisaMs = tengahMalam - sekarang;
