# Guru Module — Quick Setup (with notes)
...
mkdir -p Modules
```
2. Copy the module you want into `Modules` (e.g., copy `Modules/Guru`).
...
```
Note: The module also registers resources via `GuruServiceProvider` during `Filament::serving`. Avoid duplicates if using both discovery and provider registration.


## Migrations
Module-only:
```bash
php artisan migrate --path=Modules/Guru/Database/Migrations
```

## Seeder
Seed sample Guru data:
```bash
php artisan db:seed --class="Modules\\Guru\\Database\\Seeders\\GuruSeeder"
```
