# KalenderDidik Module — Quick Setup (with notes)
...
mkdir -p Modules
```
2. Copy the module you want into `Modules` (e.g., copy `Modules/KalenderDidik`).
...
```json
{
  "autoload": {
    "psr-4": {
      "Modules\\KalenderDidik\\": "Modules/KalenderDidik/"
    }
  }
}
```
...
```
Note: The module also registers resources via `KalenderDidikServiceProvider` during `Filament::serving`. Avoid duplicates if using both discovery and provider registration.

## Migrations
Module-only:
```bash
php artisan migrate --path=Modules/KalenderDidik/Database/Migrations
```

## Seeder
Seed sample Kaldik data:
```bash
php artisan db:seed --class="Modules\\KalenderDidik\\Database\\Seeders\\KaldikSeeder"
```
